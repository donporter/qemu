                              ...
                              bool read_only)
{
 ...
 restart_all:
    ...
    hwaddr pt_node = x86_page_table_root(cs, &layout);
    ...
    do {
        ...
        x86_get_pte(cs, pt_node, index, i, &pt_entry, 0);
        pte_addr = pt_entry.pte_addr;
    ...
    restart:
        ...
        if (pt_entry.contents & my_rsvd_mask) {
            goto do_fault_rsvd;
        }

        pte = pt_entry.contents;

        /* Check if we have hit a leaf.  Won't happen (yet) at heights > 3. */
        if (pt_entry.leaf) {
            assert(i < 4);
            page_size = pt_entry.leaf_page_size;
            ptep &= pte ^ PG_NX_MASK;
