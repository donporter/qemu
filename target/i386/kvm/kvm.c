...
static int kvm_get_nested_state(X86CPU *cpu)
{
    ...
    if (env->hflags & HF_GUEST_MASK) {
        ...
        /* Extract the EPTP value from the vmcs12 structure, and store in arch state */
        if (env->nested_state->format == KVM_STATE_NESTED_FORMAT_VMX) {
            ...
            if (vmcs->secondary_vm_exec_control & VMX_SECONDARY_EXEC_ENABLE_EPT) {
                ...
                env->enable_ept_accessed_dirty = !!(vmcs->ept_pointer & (1 << 6));
